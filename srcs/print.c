...
{
	printf("\n Sending an " COLOR_CYAN "ARP reply" COLOR_RESET " to " COLOR_RED
		   "%s (target)" COLOR_RESET " with " COLOR_RED
...
void print_sent(t_malcolm *malcolm, uint8_t src_mac[6])
{
	printf(" Sent an " COLOR_CYAN "ARP reply" COLOR_RESET " packet: " COLOR_RED
		   ...
		   malcolm->src_ip, src_mac[0], src_mac[1], src_mac[2], src_mac[3],
		   src_mac[4], src_mac[5]);
}

void print_check(void)
{
	printf("\n Please check " COLOR_CYAN "Arp table" COLOR_RESET
		   " on the " COLOR_RED "target\n" COLOR_RESET);
}
