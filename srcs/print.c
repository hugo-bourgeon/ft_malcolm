...
/*   Updated: 2025/05/21 19:00:04 by hubourge         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void print_arp_request(struct ether_arp *arp, char *ip_str)
{
	...
	printf(COLOR_RED "%02x:%02x:%02x:%02x:%02x:%02x\n" COLOR_RESET, arp->arp_sha[0], arp->arp_sha[1], arp->arp_sha[2], arp->arp_sha[3],
...
{
	printf("\n Sending an " COLOR_CYAN "ARP reply" COLOR_RESET " to " COLOR_RED "%s (target)" COLOR_RESET " with " COLOR_RED
		   "spoofed mac" COLOR_RESET ", wait...\n",
		   malcolm->trgt_ip);
}

void print_sent(t_malcolm *malcolm, uint8_t src_mac[6])
{
	printf(" Sent an " COLOR_CYAN "ARP reply" COLOR_RESET " packet: " COLOR_RED "%s is-at %02x:%02x:%02x:%02x:%02x:%02x" COLOR_RESET "\n",
